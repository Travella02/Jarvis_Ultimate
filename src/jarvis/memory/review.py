...
def _contains_internal_review_label(value: str) -> bool:
    normalized = normalize_text(str(value or ""))
    return any(label in normalized for label in _INTERNAL_REVIEW_LABELS)


def _canonical_review_key(value: str) -> str:
    normalized = normalize_text(str(value or ""))
    replacements = {
        "fiance": "fiancee",
        "fiancee": "fiancee",
    ...
    normalized = re.sub(r"\b([a-z][a-z ]{1,80}?) is your fiancee\s+\1 is your fiancee\b", r"\1 is your fiancee", normalized)
    normalized = re.sub(r"\b([a-z][a-z ]{1,80}?) is your ([a-z ]{2,30})\s+\1 is your \2\b", r"\1 is your \2", normalized)
    return normalized


def _collapse_duplicate_relationship_clauses(value: str) -> str:
    ...
    keys: set[str] = set()
    for part in parts:
        key = _canonical_review_key(part)
...
def _sentence(text: str) -> str:
    if _contains_internal_review_label(str(text or "")):
        return ""
    raw = _collapse_duplicate_relationship_clauses(str(text or ""))
    value = redact_sensitive_text(_second_person(" ".join(raw.split())), max_chars=max(len(str(text or "")), 120))
    value = value.strip(" -")
    if not value or _contains_internal_review_label(value):
        return ""
    value = re.sub(r"\b(?:fiance|fiancee|fiancees)\b", "fiancée", value, flags=re.IGNORECASE)
    value = re.sub(r"\bfiancé\b", "fiancée", value, flags=re.IGNORECASE)
    if value[-1] not in ".!?":
        value += "."
    return value
...
def _edge_sentence(edge: dict[str, Any], *, default_source: str = "") -> str:
    ...
    if _contains_internal_review_label(source):
        source = _clean_display(default_source)
...
def build_memory_review(
    ...
    *,
    ...
    def add(text: str, *, source: str, category: str = "general", importance: int = 3, score: float = 0.0, reason: str = "", metadata: dict[str, Any] | None = None) -> None:
        ...
        if not sentence:
            return
        normalized = _canonical_review_key(sentence)
    ...
    if entity_memory is not None and hasattr(entity_memory, "search"):
        ...
        for match in entity_matches:
            ...
            for key, value in sorted(attributes.items()):
                ...
                if key in {"relationship", "species", "breed", "breed_or_note"} or value in (None, "", [], {}) or _contains_internal_review_label(key_text):
                    continue
                add(f"{name} {key_text}: {value}", source="entity", category=entity_type, importance=3, score=score, reason="attribute")

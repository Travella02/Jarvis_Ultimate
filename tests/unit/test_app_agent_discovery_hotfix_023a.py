...
class TestAppAgentDiscoveryHotfix023a(unittest.TestCase):
    def test_builtin_alias_prefers_discovered_launcher_over_path_command(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            candidate = AppCandidate(
                ...
                aliases=["chrome", "google chrome", "google browser"],
                source="test_discovery",
                process_names=["chrome.exe"],
            )
            with patch.dict("os.environ", {"JARVIS_ALLOW_OS_LAUNCH_DURING_TESTS": "1"}), \
                 patch("jarvis.tools.shared.app_discovery.discover_apps", return_value=[candidate]), \
        ...
        self.assertEqual(result.data["target"], "Google Chrome")
        self.assertEqual(result.data["app_match"]["candidate"]["path"], str(chrome_path))

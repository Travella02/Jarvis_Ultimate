...
class ChatArchiveStore:
    ...
    def format_search(self, query: str, *, limit: int = 5) -> str:
        results = self.search(query, limit=limit)
        if not results:
            return f"I do not remember a recent conversation about {query}, sir."
        if len(results) == 1:
            record = results[0].record
            user = self._preview(record.user)
            assistant = self._preview(record.assistant)
            if user and assistant:
                return f"I remember that we talked about {query}, sir. You asked, ‘{user}’ and I answered, ‘{assistant}’."
            if user:
                return f"I remember you brought up {query}, sir. You said, ‘{user}’."
            return f"I remember we discussed {query}, sir."

        highlights: list[str] = []
        for item in results[:3]:
            record = item.record
            user = self._preview(record.user, limit=90)
            assistant = self._preview(record.assistant, limit=90)
            if user and assistant:
                highlights.append(f"you asked ‘{user}’ and I replied ‘{assistant}’")
            elif user:
                highlights.append(f"you said ‘{user}’")
            elif assistant:
                highlights.append(f"I said ‘{assistant}’")
        if not highlights:
            return f"I remember we talked about {query}, sir."
        return f"I remember a few recent moments about {query}, sir. " + "; ".join(highlights) + "."

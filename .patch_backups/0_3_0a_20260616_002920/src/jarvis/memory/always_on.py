...
class MemoryCandidateStore:
    ...
    def approve(self, query: str = "", *, all_matches: bool = False) -> list[MemoryCandidateRecord]:
        matches = self._select(query, all_matches=all_matches)
    ...
    def format_pending(self, *, limit: int | None = None) -> str:
        ...
        lines = [f"I have {len(pending)} memory candidate{'s' if len(pending) != 1 else ''} waiting for review, sir:"]
        for record in pending:
            lines.append(f"- {self._friendly_candidate(record)}")
        return "\n".join(lines)
    ...
    def _find_duplicate(self, normalized: str) -> MemoryCandidateRecord | None:
        for record in self._records:
            if record.normalized_text == normalized and record.status == "pending":
                return record
        return None
    ...
    def _friendly_candidate(record: MemoryCandidateRecord) -> str:
        text = ShortTermFactStore._for_user(record.text)
        tier = record.suggested_tier.replace("_", " ")
        return f"{text} ({tier}, confidence {record.confidence:.2f})"

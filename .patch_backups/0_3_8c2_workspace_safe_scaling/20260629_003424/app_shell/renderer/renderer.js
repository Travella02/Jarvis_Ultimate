...
const PANEL_LAYOUT_VIEWPORT_STORAGE_KEY = 'jarvis.appShell.panelLayoutViewport.v038c1';
...
function currentPanelViewportSnapshot() {
  return {
    width: Math.max(PANEL_MIN_FLOATING_WIDTH + PANEL_VIEWPORT_MARGIN * 2, window.innerWidth || document.documentElement.clientWidth || PANEL_MIN_FLOATING_WIDTH),
    height: Math.max(PANEL_MIN_RESTORED_HEIGHT + PANEL_VIEWPORT_MARGIN * 2, window.innerHeight || document.documentElement.clientHeight || PANEL_MIN_RESTORED_HEIGHT)
  };
}
...
function normalizePanelViewportSnapshot(value) {
  ...
  if (!Number.isFinite(width) || !Number.isFinite(height) || width < 1 || height < 1) {
    return currentPanelViewportSnapshot();
  }
  return { width, height };
}
...
function inferPanelLayoutViewport() {
  ...
  let inferredWidth = current.width;
  let inferredHeight = current.height;
  for (const record of Object.values(panelLayout || {})) {
    ...
    if (Number.isFinite(right)) inferredWidth = Math.max(inferredWidth, right);
    if (Number.isFinite(bottom)) inferredHeight = Math.max(inferredHeight, bottom);
  }
...
function scaleFloatingLayoutForViewport(previousSnapshot, nextSnapshot) {
  ...
  for (const key of Object.keys(panelLayout)) {
    ...
    const scaled = {
      ...record,
      x: next.margin + (Number(record.x || previous.margin) - previous.margin) * ratioX,
      y: next.margin + (Number(record.y || previous.margin) - previous.margin) * ratioY,
...
function applyPanelLayout() {
  for (const panel of layoutPanels()) {
    ...
    const key = panelKeyFromElement(panel);
    const record = sanitizeLayoutRecord(panelLayout[key] || {}, key);

...
function requestCustomPresetName(defaultName, options = {}) {
  ...
  const dialogHint = options.hint || 'Save the current panel positions, sizes, locks, and z-order as a selectable custom preset.';
...
async function saveCurrentLayoutPreset() {
  ...
  try {
    const entered = await requestCustomPresetName(nextCustomPresetDefaultName(), {
      ...
      hint: 'Save the current panel positions, sizes, locks, and z-order as a selectable custom preset.',

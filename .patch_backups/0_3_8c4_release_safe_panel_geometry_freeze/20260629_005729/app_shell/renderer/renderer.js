...
function capturePanelInteractionLayout(activeKey) {
  const snapshot = {};
  for (const [key, record] of Object.entries(panelLayout || {})) {
    if (key === activeKey) continue;
    snapshot[key] = clonePanelLayoutRecord(record);
  }
  return snapshot;
}
...
function applyPanelLayout(options = {}) {
  ...
  for (const panel of layoutPanels()) {
    ...
    const record = preservedKeys.has(key) && existing.mode === 'floating'
      ? { ...sanitizeLayoutRecord({ ...existing, mode: 'floating' }, key), ...existing, mode: 'floating' }
...
// 0.3.8c3: active panel interactions freeze every other panel so moving the orb never reflows the conversation panel.

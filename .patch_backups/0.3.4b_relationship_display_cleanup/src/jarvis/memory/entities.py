...
class EntityMemoryStore:
    ...
    def _normalize_loaded_record(self, record: EntityRecord) -> EntityRecord:
        ...
        if isinstance(record.attributes, dict):
            ...
            if relation:
                record.attributes["relationship"] = _normalize_relationship_label(str(relation))
        ...
        if record.entity_type == "person" and isinstance(record.attributes, dict):
            relation = record.attributes.get("relationship")
            if relation:
                normalized_relationships.append({"type": _normalize_relationship_label(str(relation)), "to": "user", "source": "attribute_backfill"})

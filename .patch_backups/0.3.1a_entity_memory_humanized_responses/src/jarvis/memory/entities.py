...
class EntityMemoryStore:
    ...
    def relevant_context(self, query: str, *, limit: int | None = None) -> str:
        ...
        for result in results:
            ...
            lines.append(f"- {record.name} ({record.entity_type}): {summary}")
        return "\n".join(lines)
    ...
    def forget(self, query: str, *, entity_type: str | None = None, limit: int | None = None) -> list[EntityRecord]:
        ...
        if not normalized:
            return []
        matches = self.search(query, entity_type=entity_type, limit=limit or 10)
        if not matches:
            return []
        remove_ids = {match.record.id for match in matches}
        removed = [record for record in self._records if record.id in remove_ids]
    ...
    def format_records(self, *, query: str = "", entity_type: str | None = None, limit: int = 8) -> str:
        ...
        for record in records:
            ...
            summary = record.summary or f"remembered {label}"
    ...
    def _dedupe_loaded_records(self, records: list[EntityRecord]) -> list[EntityRecord]:
        selected: dict[tuple[str, str], EntityRecord] = {}
        for record in records:
            key = (record.entity_type, record.normalized_name)
...
def infer_entity_from_text(text: str) -> dict[str, Any] | None:
    ...
    relationship_words = "fiance|fiancée|wife|girlfriend|brother|sister|mom|mother|dad|father|friend|coworker|teammate|lead developer|developer"
    match = re.search(rf"\b([A-Z][A-Za-z0-9_\-']{{1,40}})\s+is\s+my\s+({relationship_words})\b", cleaned, flags=re.IGNORECASE)
    if match:
        name = _clean_title(match.group(1))
        relation = _clean_title(match.group(2).lower())
        return {
            "name": name,
            "entity_type": "person",
            "summary": f"{name} is the user's {relation}.",
    ...
    match = re.search(rf"\bmy\s+({relationship_words})\s+is\s+([A-Z][A-Za-z0-9_\-']{{1,40}})\b", cleaned, flags=re.IGNORECASE)
    if match:
        relation = _clean_title(match.group(1).lower())
    ...
    if match:
        ...
        return {
            "name": name,
            "entity_type": "pet",
            "summary": f"{name} is the user's {species}." + (f" {breed_or_note}." if breed_or_note else ""),
    ...
    if match:
        ...
        return {
            "name": name,
            "entity_type": "pet",
            "summary": f"{name} is the user's {species}.",
    ...
    if match:
        ...
        return {
            "name": name,
            "entity_type": "app",
            "summary": f"{name} is the user's {role.replace('_', ' ')} app.",
    ...
    if match:
        ...
        return {
            "name": name,
            "entity_type": "vehicle",
            "summary": f"The user's {kind} is {name}.",
            "attributes": attributes,
            "tags": ["vehicle", kind],
            "importance": 3,
            "confidence": 0.75,
    ...
    if match:
        ...
        return {
            "name": name,
            "entity_type": "device",
            "summary": f"The user's {kind} is {detail}.",

...
const PANEL_MIN_FLOATING_WIDTH = 240;
const PANEL_MIN_FLOATING_HEIGHT = 58;
const PANEL_MIN_RESTORED_HEIGHT = 120;
...
function viewportPanelBounds() {
  const width = Math.max(PANEL_MIN_FLOATING_WIDTH + PANEL_VIEWPORT_MARGIN * 2, window.innerWidth || document.documentElement.clientWidth || PANEL_MIN_FLOATING_WIDTH);
  const height = Math.max(PANEL_MIN_RESTORED_HEIGHT + PANEL_VIEWPORT_MARGIN * 2, window.innerHeight || document.documentElement.clientHeight || PANEL_MIN_RESTORED_HEIGHT);
  return {
    width,
    height,
...
function sanitizeLayoutRecord(record) {
  ...
  const minimumHeight = minimized ? PANEL_MIN_FLOATING_HEIGHT : PANEL_MIN_RESTORED_HEIGHT;
  const width = clamp(Number(record?.width || 340), PANEL_MIN_FLOATING_WIDTH, bounds.maxPanelWidth);
  const height = clamp(Number(record?.height || 240), PANEL_MIN_FLOATING_HEIGHT, bounds.maxPanelHeight);
  const maxX = Math.max(bounds.margin, bounds.width - width - bounds.margin);
  const maxY = Math.max(bounds.margin, bounds.height - height - bounds.margin);
  return {
    mode: record?.mode === 'floating' ? 'floating' : 'docked',
    x: clamp(Number(record?.x || 16), bounds.margin, maxX),
    y: clamp(Number(record?.y || 96), bounds.margin, maxY),
    width,
    height: minimized ? Math.max(PANEL_MIN_FLOATING_HEIGHT, Math.min(height, bounds.maxPanelHeight)) : Math.max(minimumHeight, height),
    minimized,
...
function clampPanelLayoutToViewport() {
  ...
  for (const key of Object.keys(panelLayout)) {
    ...
    const after = sanitizeLayoutRecord(panelLayout[key] || {});
...
function scheduleResponsiveLayoutClamp() {
  ...
  responsiveLayoutClampRaf = window.requestAnimationFrame(() => {
    ...
    if (document.querySelector('.dockable-panel.is-dragging, .dockable-panel.is-resizing')) {
      scheduleResponsiveLayoutClamp();
      return;
    }
    removePanelLayoutPlaceholder(activePanelPlaceholder);
    clampPanelLayoutToViewport();
    applyPanelLayout();
  });
}
...
function beginPanelDrag(event, panel) {
  ...
  const up = () => {
    ...
    panelLayout[key] = sanitizeLayoutRecord(panelLayout[key] || {});
    savePanelLayout();
    applyPanelLayout();
